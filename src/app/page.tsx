    ...
    <>
      ...
      <div className=" my-24 py-12 bg-hijau w-full h-300 ">
        ...
        <div className="  flex flex-col xl:w-3/5 w-4/5 mx-auto gap-10 justify-around  items-center ">
          {programGet.map((res, loop) => {
            if (res.deskripsi !== "tidak ada isinya") {
              if (loop <= 1) {
                return (
                  <div key={res.id} className="flex my-10 w-full mx-auto gap-10 justify-center">
                    <article className="hidden md:flex w-1/2 h-1/2 mt-[25px] text-wrap-n text-center">
                      <p className="text-white leading-8 font-light text-[14px] lg:text-[15px] ">{res.deskripsi}</p>
                    </article>
